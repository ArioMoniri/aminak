#!/usr/bin/env python3
"""Stage 8 v4: Analysis with low_confidence filter (Fix E), Spearman rank
correlation (Fix F), WT reference rows (Fix K), Vina noise floor (Fix L).
"""
import os, json, math
...
STAGE_LOG = os.path.join(LOG_DIR, "v4_08_analysis.log")

VINA_NOISE_FLOOR = 0.85


def log(msg):
    line = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [V4] STAGE8: {msg}"
...
def main():
    ...
    log("Stage 8 v4 starting")

    df = pd.read_csv(os.path.join(MUT_DIR, "mutant_results_v4.csv"), comment="#")
    ...
    # Pivot — exclude the WT row from analysis but keep accessible
    ...
    pv.columns = ["_".join([str(x) for x in c if x is not None and str(x) != ""]).strip("_")
                  for c in pv.columns]
    pv.to_csv(os.path.join(ANA_DIR, "mutant_pivot_v4.csv"), index=False)
    ...
    # Top destabilisers — exclude mis_docked AND low_confidence (Fix E)
    # Special case: in v4 the WT holo dock itself migrated out of the crystal
    # pocket (the ionised cofactor displaces dUMP). When the WT reference is in
    # the new pose, mutants converging on the same new pose are still self-
    # consistent. If WT holo is itself "mis-docked" vs the crystal pocket, we
    # redefine mis_docked for holo as "RMSD differs from WT holo RMSD by >3 A"
    # (i.e. the dock landed in a different pocket from WT, not just outside the
    # crystal pose).
    ...
    if not math.isnan(wt_holo_rmsd) and wt_holo_rmsd > 3.0:
        ...
        holo_filter_note = (f"WT holo itself is RMSD={wt_holo_rmsd:.2f} A vs crystal "
                            f"(ionised cofactor displaces dUMP). Holo mis_docked "
                            f"redefined as |RMSD - WT_holo_RMSD| > 3 A.")
    ...
    log(f"top apo destab (well-docked, n>=5): "
        f"{df_apo_clean[['mutant','delta_vina_vs_wt','rmsd_to_native','n_modes']].head(5).values.tolist()}")
    log(f"top holo destab (well-docked, n>=5): "
        f"{df_holo_clean[['mutant','delta_vina_vs_wt','rmsd_to_native','n_modes']].head(5).values.tolist()}")

    # Build per-mutant aligned arrays
    mutants_ordered = df_apo.mutant.tolist()
    apo_d, holo_d = [], []
    apo_mis, holo_mis = [], []
    apo_lowc, holo_lowc = [], []
    # Use the (possibly relaxed) df_holo for holo flags
    ...
    # Spearman restricted to |delta|>0.3 in either condition (Fix F)
    ...
    # Plot 1: bar chart Δ apo vs holo
    ...
    ax.set_ylabel("Δ Vina score vs WT v4 (kcal/mol; positive = destabilising)")
    ax.set_title("v4 mutant docking: Δ Vina score vs WT (apo vs holo)")
    ...
    ax.set_title("v4 Apo vs Holo Δ Vina (well-docked, n_modes≥5)")
    txt = []
    if pearson_r is not None:
        txt.append(f"Pearson r = {pearson_r:.2f} (n={int(valid.sum())})")
    if spearman_r is not None:
        txt.append(f"Spearman ρ = {spearman_r:.2f}")
    if spearman_r_filt is not None:
        txt.append(f"Spearman ρ (|Δ|>0.3) = {spearman_r_filt:.2f} (n={n_filt})")
    ...
    # Plot 3: category boxplot — use the relaxed holo filter and the apo as-is
    ...
    ax.set_ylabel("Δ Vina score vs WT v4 (kcal/mol)")
    ax.set_title("v4 Δ Vina by mutation category (excludes mis-docked & n_modes<5)")
    ...
    summary = {
        ...
        "wt_apo_aff_v4": float(df_wt[df_wt.condition == "apo"].top_affinity.iloc[0])
                         if (df_wt.condition == "apo").any() else None,
        "wt_holo_aff_v4": float(df_wt[df_wt.condition == "holo"].top_affinity.iloc[0])
                          if (df_wt.condition == "holo").any() else None,
        "wt_holo_n_modes_v4": int(df_wt[df_wt.condition == "holo"].n_modes.iloc[0])
                              if (df_wt.condition == "holo").any() else None,
        ...
        "sign_convention": "delta_vina_vs_wt = top_aff_mut - top_aff_wt_v4; positive = destabilising",
    }
    with open(os.path.join(ANA_DIR, "summary_v4.json"), "w") as f:
        json.dump(summary, f, indent=2, default=str)
    log(f"summary written")
    log(f"Pearson r = {pearson_r}, Spearman ρ = {spearman_r}, "
        f"filtered Spearman ρ = {spearman_r_filt} (n={n_filt})")
    log("Stage 8 v4 DONE")
