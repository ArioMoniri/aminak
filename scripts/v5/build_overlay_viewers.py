...
<div class="panel">
 ...
 <div class="controls">
  ...
  <button onclick="setCartoon()">Cartoons only</button>
  <button onclick="setRibbon()">Ribbon</button>
  <button onclick="setBackbone()">Cα trace</button>
  <button onclick="zoomAll()">Zoom to fit</button>
...
viewer.addModel(crystalText, "pdb");   // model index 0 — crystal
viewer.addModel(modelText, "pdb");     // model index 1 — Modeller model

function setCartoon() {{
  viewer.setStyle({{model:0}}, {{cartoon: {{color: '#d62895', opacity: 0.85}}}});
  viewer.setStyle({{model:1}}, {{cartoon: {{color: '#2ca02c', opacity: 0.85}}}});
  viewer.render();
}}
function setRibbon() {{
  viewer.setStyle({{model:0}}, {{cartoon: {{style:'ribbon', color:'#d62895'}}}});
  viewer.setStyle({{model:1}}, {{cartoon: {{style:'ribbon', color:'#2ca02c'}}}});
  viewer.render();
}}
function setBackbone() {{
  viewer.setStyle({{model:0}}, {{line: {{color:'#d62895', linewidth:2.5}}}});
  viewer.setStyle({{model:1}}, {{line: {{color:'#2ca02c', linewidth:2.5}}}});
  viewer.render();
}}
function zoomAll() {{ viewer.zoomTo(); viewer.zoom(0.9); viewer.render(); }}
function spinOn()  {{ viewer.spin(true); }}
function spinOff() {{ viewer.spin(false); }}
setCartoon();
zoomAll();
viewer.render();
</script>
