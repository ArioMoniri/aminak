#!/usr/bin/env python3
"""Stage 7 v4: Re-dock 20 mutants HOLO using new (Fix-A) cofactor receptor.
Reuse apo dockings from v3 verbatim (receptor unchanged in apo).
Preserve UMP atom names through the pipeline (Fix D).
"""
...
MUT_V4 = os.path.join(PROJECT, "07d_mut_docking_v4")
VIEWER_DIR = os.path.join(MUT_V4, "viewer_files")
WT_V4 = os.path.join(PROJECT, "06d_docking_wt_v4")
WT_V3 = os.path.join(PROJECT, "06c_docking_wt_v3")
...
STAGE_LOG = os.path.join(LOG_DIR, "v4_07_mutants.log")


def log(msg):
    line = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [V4] STAGE7: {msg}"
...
def main():
    os.makedirs(MUT_V4, exist_ok=True)
    os.makedirs(VIEWER_DIR, exist_ok=True)
    open(STAGE_LOG, "w").close()
    log("Stage 7 v4 starting")

    # List mutants from v3
    ...
    wt_apo = json.load(open(os.path.join(WT_V4, "wt_apo.json")))
    wt_holo = json.load(open(os.path.join(WT_V4, "wt_holo.json")))
    wt_apo_aff = wt_apo["top_affinity"]
    wt_holo_aff = wt_holo["top_affinity"]
    log(f"WT v4 apo aff = {wt_apo_aff:.2f}, WT v4 holo aff = {wt_holo_aff:.2f}")

    # Read v3 csv to inherit category info
    v3_csv = os.path.join(MUT_V3, "mutant_results_v3.csv")
    cat_map = {}
    apo_v3_map = {}  # mutant -> v3 apo row dict
    ...
    for mi, mid in enumerate(mutants):
        ...
        sub = os.path.join(MUT_V4, mid)
        os.makedirs(sub, exist_ok=True)

        # 1) Reuse v3 mutated structure (mut_h.pdb)
        ...
        if not os.path.exists(v3_mut_h):
            ...
            continue
        v4_mut_h = os.path.join(sub, f"{mid}_mut_h.pdb")
        shutil.copy(v3_mut_h, v4_mut_h)

        # 2) Build holo receptor with v4 cofactor
        holo_pdb = os.path.join(sub, f"{mid}_holo.pdb")
        make_holo_dimer(v4_mut_h, cof_a_v4, cof_b_v4, holo_pdb)
        rec_holo = os.path.join(sub, f"{mid}_holo.pdbqt")
        ok_holo, m_holo = prepare_receptor_with_charges(holo_pdb, rec_holo, f"{mid}_holo", log_fn=log)
        if not ok_holo:
        ...
        # 3) Dock holo
        out_pdbqt = os.path.join(sub, f"{mid}_holo.pdbqt_out")
        log_file = os.path.join(sub, f"{mid}_holo.log")
        # rename to avoid collision with input pdbqt
        ...
        # 4) Reuse v3 apo result (just rewrite delta against WT v4 apo)
        if mid in apo_v3_map:
            ...
            low_confidence_apo = n_modes_apo < 5  # informational; should be False
            delta_apo = (top_apo - wt_apo_aff) if not math.isnan(top_apo) else float("nan")

            # Copy viewer file paths from v3
            v3_complex = v3a.get("complex_pdb", "")
            v3_top_pose = v3a.get("top_pose_pdb", "")
            # Symlink to v4 viewer dir for self-containment
            v4_top = os.path.join(VIEWER_DIR, f"{mid}_apo_top_pose.pdb")
            v4_cmp = os.path.join(VIEWER_DIR, f"{mid}_apo_complex.pdb")
            if os.path.exists(v3_top_pose) and not os.path.exists(v4_top):
                shutil.copy(v3_top_pose, v4_top)
            if os.path.exists(v3_complex) and not os.path.exists(v4_cmp):
                shutil.copy(v3_complex, v4_cmp)
            ...
            rows.append({"mutant": mid, "category": cat, "condition": "apo",
                         ...
                         "complex_pdb": v4_cmp, "top_pose_pdb": v4_top,
    ...
    csv_path = os.path.join(MUT_V4, "mutant_results_v4.csv")
    with open(csv_path, "w", newline="") as f:
        f.write("# Sign convention: delta_vina_vs_wt = top_aff_mut - top_aff_wt_v4; positive = destabilising.\n")
        f.write("# mean_topk = mean(affinities[:min(3, n_modes)]).\n")
        f.write("# low_confidence = (n_modes < 5). Excluded from rankings.\n")
        f.write("# Apo dockings reused from v3 (apo receptor unchanged).\n")
        # Add WT reference rows (Fix K)
        wt_apo_row = {"mutant": "WT", "category": "wildtype", "condition": "apo",
                      ...
                      "mis_docked": False, "low_confidence": (wt_apo.get("n_modes", 0) < 5),
                      "n_clashes": 0,
        ...
        wt_holo_row = {"mutant": "WT", "category": "wildtype", "condition": "holo",
                       ...
                       "delta_vina_vs_wt": 0.000,
                       "mis_docked": False, "low_confidence": (wt_holo.get("n_modes", 0) < 5),
                       "n_clashes": 0,
    ...
    with open(os.path.join(MUT_V4, "summary_v4.json"), "w") as f:
        json.dump({
            "n_mutants": len(mutants),
            "n_rows": len(rows),
            "skipped": skipped,
            "wt_apo_v4": wt_apo_aff,
            "wt_holo_v4": wt_holo_aff,
            "wt_holo_v4_n_modes": wt_holo.get("n_modes", 0),
            "wt_holo_v4_best_seed": wt_holo.get("best_seed"),
    ...
    log(f"Stage 7 v4 DONE: {len(rows)} rows, {len(skipped)} skipped")
